...
parser.add_argument('-i', '--input', help='input json file generated by crossroad describer', type=argparse.FileType('r'), required=True)
parser.add_argument('-d', '--display', help='display a preview of the crossroad schematization', action='store_true')
parser.add_argument('-o', '--output', help='output file (supported format: svg)', type=argparse.FileType('w'))
    ...
    def is_intersecting_osm(self):
        for n1 in self.osm_input:
            for n2 in self.osm_input[n1]:
                if Utils.is_roadway_edge(self.osm_input[n1][n2][0]):
...
class Crossing:
    ...
    def get_adjacent_roadways_nodes(self):
        return [x for x in self.osm_input[self.node_id] if Utils.is_roadway_edge(self.osm_input[self.node_id][x][0])]


    def get_adjacent_footways_nodes(self):
        return [x for x in self.osm_input[self.node_id] if Utils.is_footway_edge(self.osm_input[self.node_id][x][0])]
    ...
    def build_vectors(self, nodes):
        return [Utils.normalized_vector(self.osm_input.nodes[self.node_id], self.osm_input.nodes[n]) for n in nodes]
...
class CrossroadSchematization:
    ...
    def load_osm(self):
        ...
        osmnx.config(use_cache=True, 
                     useful_tags_node = list(set(osmnx.settings.useful_tags_node + CrossroadSchematization.node_tags_to_keep)))
    ...
    def build_sidewalks(self):
        ...
        for bid in self.branches:
            ...
            if len(wbranches) == 1 and wbranches[0][1]["left_sidewalk"] != "" and wbranches[0][1]["right_sidewalk"] != "":
                ...
                self.sidewalks[bid] = self.build_two_sidewalks(wbranches[0][0].edge, width + 2 * self.distance_kerb_footway, 
                                                               wbranches[0][1], so,
                                                               wbranches[0][1], so)
    ...
    def assemble_sidewalks(self):
        ...
        for sid in original_sidewalks_ids:
            self.merged_sidewalks.append(TurningSidewalk(self.get_sidewalks_by_id(sid), self.osm_input, self.osm_buffer_for_bevel))
...
print("Loading input geojson (" + args.input.name + ")")
cr_input = geopandas.read_file(args.input)
...
if args.output:
    if len(args.output.name) < 4:
        "Cannot deduce required format with small file names"
        
if args.output and args.output.name[:-3] == "svg":
    print("Exporting as svg:", args.output.name)
    # TODO
